...
Shader "Universal Render Pipeline/Custom/CurvedTerrain"
{
    Properties
    {
        ...
        _Curvature ("Curvature", float) = .5
        _CurvePosition ("Curvature Origin", Vector) = (0,0,0,0)
    }
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            Varyings LitPassVertex(Attributes input)
            {
                ...
                float4 worldSpaceVertex = mul(unity_ObjectToWorld, input.positionOS);
                float4 cameraPos = _CurvePosition;
                float4 diff = worldSpaceVertex - cameraPos;
                worldSpaceVertex.y += ((diff.x * diff.x) * -1) * _Curvature;
                vertexInput.positionWS = worldSpaceVertex;
                vertexInput.positionCS = mul(UNITY_MATRIX_VP, worldSpaceVertex);
        ...
        Pass
        {
            ...
            float4 GetShadowPositionHClip(Attributes input)
            {
                ...
                float3 positionWS = worldSpaceVertex;
        ...
        Pass
        {
            ...
            Varyings DepthOnlyVertex(Attributes input)
            {
                ...
                output.uv = TRANSFORM_TEX(input.texcoord, _BaseMap);
                float4 worldSpaceVertex = mul(unity_ObjectToWorld, input.position);
                float4 cameraPos = _CurvePosition;
                float4 diff = worldSpaceVertex - cameraPos;
                worldSpaceVertex.y += ((diff.x * diff.x) * -1) * _Curvature;

                output.positionCS = mul(UNITY_MATRIX_VP, worldSpaceVertex);
    ...
    // same properties.
    // Can't use the custom editor since I added new properties
